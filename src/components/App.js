...
const App = () => {
  ...
  useEffect(() => {
    async function getInitialWeatherData() {
      const DEFAULT_LATITUDE = -33.934444,
        DEFAULT_LONGITUDE = 18.869167; // Stellenbosch set as default.
      ...
      fetch(
        ...
        .catch((err) => {
          ...
          if (lat === DEFAULT_LATITUDE && long === DEFAULT_LONGITUDE)
            setCurrentLocation("Stellenbosch");
      ...
      setWeatherApi(
        `${proxy}https://api.darksky.net/forecast/6385bf526a557ab35c6534562b693fdc/${lat},${long}`
      );
    }
  ...
  function setLastUpdateTime() {
    ...
    setUpdateTime(
      `Last updated: ${hours < 10 ? "0" + hours : hours}:${
  ...
  function setTheme() {
    const dayTheme = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue("--bg-day");
    const nightTheme = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue("--bg-night");
    ...
    if (Object.keys(daily).length !== 0) {
      ...
      return timeNow < sunrise_time || timeNow > sunset_time
        ? nightTheme
        : dayTheme;
    }
    ...
    <>
      ...
      <div className="App">
        <Container>
          ...
          />
          <Summary currentLocation={currentLocation} updateTime={updateTime} />
          <hr />
          <HourlyForecast
